...
from test_framework.mininode import mergexhash
from test_framework.util import (assert_equal,
                                 bytes_to_hex_str,
                                 hash256,
                                 )

def mergexhash_helper(b):
    return encode(mergexhash(b)[::-1], 'hex_codec').decode('ascii')
...
class ZMQTest (BitcoinTestFramework):
    ...
    def _zmq_test(self):
        ...
        for x in range(num_blocks):
            ...
            assert_equal(genhashes[x], mergexhash_helper(block[:80]))
