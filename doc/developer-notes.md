...
- [Development guidelines](#development-guidelines)
    - [General MERGEX Core](#general-mergex-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
MERGEX Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/mergexpay/mergex/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_mergex
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_mergex --log_level=test_suite
$ valgrind -v --leak-check=full src/mergexd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_mergex.coverage/index.html`.
```

**Sanitizers**

MERGEX Core can be compiled with various "sanitizers" enabled, which add
...
- ThreadSocketHandler : Sends/Receives data from peers on port 3381.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 4490 for connections and services them.
...
However, in open source software such as MERGEX Core, where everyone uses
...
pay attention to for reviewers of MERGEX Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: MERGEX Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against MERGEX Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `mergex-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `mergex-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of MERGEX Core, the wallet was always
