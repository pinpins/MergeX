GENERIC BUILD NOTES
====================
Some notes on how to build MERGEX Core based on the [depends](../depends/README.md) build system.

Note on old build instructions
------------------------------
In the past, the build documentation contained instructions on how to build MERGEX with system-wide installed dependencies
...
------------------------------------
Building the dependencies and MERGEX Core requires some essential build tools to be installed before. Please see
[build-unix](build-unix.md), [build-osx](build-osx.md) and [build-windows](build-windows.md) for details.

Building dependencies
---------------------
MERGEX inherited the `depends` folder from Bitcoin, which contains all dependencies required to build MERGEX. These
dependencies must be built before MERGEX can actually be built. To do so, perform the following:
...
This will download and build all dependencies required to build MERGEX Core. Caching of build results will ensure that only
...
not done, build failures might occur when building MERGEX.
...
ccache
------
`./configure` of MERGEX Core will autodetect the presence of ccache and enable use of it. To disable ccache, use
