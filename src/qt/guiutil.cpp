...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent, bool fAllowURI)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a MERGEX address (e.g. %1)").arg(
...
void setupAppearance(QWidget* parent, OptionsModel* model)
{
    if (!QSettings().value("fAppearanceSetupDone", false).toBool()) {
        ...
        dlg.setWindowIcon(QIcon(":icons/mergex"));
...
{
    // return if URI is not valid or is no mergex: URI
    if(!uri.isValid() || uri.scheme() != QString("mergex"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::MERGEX, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    QString ret = QString("mergex:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::MERGEX, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open mergex.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "MERGEX Core.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "MERGEX Core (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("MERGEX Core (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for "MERGEX Core*.lnk"
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "mergex.desktop";
    return GetAutostartDir() / strprintf("mergex-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a mergex.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=MERGEX Core\n";
        else
            optionFile << strprintf("Name=MERGEX Core (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the MERGEX Core app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add MERGEX Core app to startup item list
...
void disableMacFocusRect(const QWidget* w)
{
    ...
    for (const auto& c : w->findChildren<QWidget*>()) {
        if (c->testAttribute(Qt::WA_MacShowFocusRect)) {
            c->setAttribute(Qt::WA_MacShowFocusRect, !mergexThemeActive());
...
void updateMacFocusRects()
{
    ...
    while (it != setRectsDisabled.end()) {
        if (allWidgets.contains(*it)) {
            (*it)->setAttribute(Qt::WA_MacShowFocusRect, !mergexThemeActive());
            ++it;
        } else {
