...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // users ignore all instructions on mergexcentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in mergex.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.pushKV("mergex.conf", statusObj);
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by masternode configured in mergex.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    /* MERGEX features */
    { "mergex",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "mergex",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "mergex",               "gobject",                &gobject,                {} },
    { "mergex",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
